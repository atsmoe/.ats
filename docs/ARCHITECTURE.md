...
截至 V2.8.25，本仓库 `index.njk` 仍加载 `star-map-3d.js`，由 `star-map-entry.js` 打包 Three.js 星图。页面保留三个世界的静态兜底入口。二维固定底图属于 B5 公开预览，不应写成正式首页已采用的实现。

世界页使用独立 `bundle.js`，不包含 Three.js。正式首页的 `init` / `destroy` 管理单一动画帧、监听器、入场定时器及 GPU 资源；重复初始化先释放旧实例。隐藏或进入 bfcache 时暂停，最终离开时销毁；减少动态效果时只在交互、resize 和恢复可见后重绘。初始化失败保留静态入口，详见 [T02 验收记录](STAR_MAP_LIFECYCLE_ACCEPTANCE.md)。
...
| 2026-09-18 | V2.8.25 | OpenAI Codex | 补齐正式星图销毁重建、隐藏暂停、bfcache 与减少动态效果的生命周期边界。 | 执行 T02 / T06，保持视觉、资料和依赖版本不变。 |
