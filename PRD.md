...
V1版本 是单页应用（index.html 2240 行），所有 CSS / JS / 数据内联在一个文件中。随着加入第二个世界（战锤 40K）和更多 IF 分支线，单页架构暴露出以下问题：
...
- JSON 数据构建时内联（file:// 兼容），开发时 fetch 热加载
...
dist/
  index.html (14KB)
  arknights.html (184KB, 含内联 JSON)
  wh40k.html (42KB, 含内联 JSON)
  about.html (11KB)
  css/ (7 个文件)
  js/
    virtual-timeline.js
...
世界页 (arknights.html / wh40k.html)：
  ...
  → loadTimelineData()（优先 window.__WORLD_DATA__ 内联数据，否则 fetch）
...
- file:// 协议：dist/ 下 HTML 直接双击打开即可使用（JSON 数据已内联，无需 fetch）
- 路径策略：全部相对路径（./js/bundle.js, ./arknights.html），兼容 GitHub Pages 等任意部署子路径
...
1. 数据加载：JSON 格式合法性、必需字段完整性、内联数据与 fetch 路径一致性
...
7. 构建产物：dist/ 输出文件完整性、file:// 协议可用性
...
- **数据**：内联 JS 变量回退 → 构建内联 window.__WORLD_DATA__ + fetch 回退
